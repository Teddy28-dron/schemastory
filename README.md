...
```bash
npx schemastory before.jsonl after.jsonl
```

Create and open a visual report:

```bash
npx schemastory before.jsonl after.jsonl --format html --output report.html --open
```
...
Version `0.1.0` implements the complete first-use loop: JSON/JSONL/NDJSON input, observed profiles, drift classification, three report formats, policy exits, examples, and privacy regression tests. It is ready for local evaluation but has not been published to GitHub or npm from this workspace.
